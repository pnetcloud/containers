...
if path.name == "build.yml":
    require("cloudnative-pg-timescaledb/scripts/ci-apt-install.sh binfmt-support qemu-user-static" in text, "workflow installs qemu packages through retrying apt helper", "retrying apt helper missing", "Use ci-apt-install.sh so transient apt mirror failures do not fail release builds.")
    require("cloudnative-pg-timescaledb/scripts/ci-retry.sh bash -c" in text and "docker login ghcr.io" in text and "GHCR_TOKEN" in text, "workflow retries GHCR login for release jobs", "GHCR login retry missing", "Use ci-retry.sh for GHCR authentication so transient registry login failures do not fail release builds.")
    require('cloudnative-pg-timescaledb/scripts/ci-retry.sh docker pull --platform "${platform}" "${candidate_ref}"' in text, "candidate smoke pulls retry transient GHCR reads", "candidate pull retry missing", "Use ci-retry.sh for candidate docker pulls before smoke checks.")
    require("CI_RETRY_ATTEMPTS=2 CI_RETRY_DELAY_SECONDS=30 cloudnative-pg-timescaledb/scripts/ci-retry.sh" in text and "docker buildx bake --file cloudnative-pg-timescaledb/docker-bake.hcl" in text, "candidate Buildx registry push has bounded retry", "Buildx Bake retry missing", "Retry the expensive candidate push once for transient BuildKit or GHCR failures without widening final tag promotion.")
    require("sudo apt-get update" not in text and "sudo apt-get install" not in text, "workflow does not use bare apt-get in release build jobs", "bare apt-get found", "Use ci-apt-install.sh for runner package installation.")
...
if path.name == "build.yml":
    require("Check vulnerability scan gate" in text and "check-vulnerability-scan-gate.py" in text and "Skipping release row because vulnerability scan did not pass" in text, "downstream release rows skip failed vulnerability scans without blocking siblings", "scan row skip marker missing", "Publish only rows whose scan summary passed and belongs to the same candidate metadata.")
...
if path.name == "build.yml":
    ...
    require("RELEASE_DATE: ${{ matrix.release_date }}" in text and "RELEASE_DATE: 20260609" not in text, "tag validation uses matrix release_date, not a hard-coded fixture date", "missing matrix release_date env", "Carry the metadata-derived release date through the matrix so future automated releases validate their own immutable tags.")
    require('validate-tags.sh --metadata cloudnative-pg-timescaledb/versions.yaml --date "${RELEASE_DATE}"' in text, "tag validation command consumes matrix release_date", "validate-tags does not use RELEASE_DATE", "Do not leave a matrix release_date env unused or replace it with a hard-coded tag validation date.")
