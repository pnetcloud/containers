...
require("Install pinned cosign with retry" in evidence and "cloudnative-pg-timescaledb/scripts/install-cosign.sh" in evidence, "workflow installs pinned cosign through retrying helper", "missing retrying cosign install helper", "Install cosign through the local helper so GitHub release download failures are retried and the pinned checksum is verified.")
