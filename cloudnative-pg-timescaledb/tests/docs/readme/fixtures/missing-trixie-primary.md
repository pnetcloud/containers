...
Barman support uses the CloudNativePG Barman Cloud Plugin path; legacy in-image `barman-cloud` binaries are not part of v1. Supported PostgreSQL majors are PostgreSQL `17`, PostgreSQL `18`, and experimental PostgreSQL `19beta1`. Debian `trixie` and Debian `bookworm` are supported.
