...
`latest` is convenience-only and points to PostgreSQL `18` on Debian `trixie`. Debian `bullseye` is supported in v1. Alpine, legacy PostgreSQL majors, and Artifact Hub metadata are out of scope for v1.
