...
`latest` is convenience-only and points to PostgreSQL `18` on Debian `bookworm`. Alpine, Debian `bullseye`, legacy PostgreSQL majors, and Artifact Hub metadata are out of scope for v1.
