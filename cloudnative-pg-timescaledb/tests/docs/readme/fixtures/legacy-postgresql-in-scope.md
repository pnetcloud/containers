...
`latest` is convenience-only and points to PostgreSQL `18` on Debian `trixie`. PostgreSQL `16` is supported for compatibility. Alpine, Debian `bullseye`, and Artifact Hub metadata are out of scope for v1.
