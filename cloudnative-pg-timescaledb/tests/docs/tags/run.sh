...
text = doc.read_text()
stable_immutable = r"(?:18-pg18\.4-ts2\.27\.2-20260609|18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>)"
stable_bookworm = r"(?:18-pg18\.4-ts2\.27\.2-20260609-bookworm|18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>-bookworm)"
pg19_immutable = r"(?:19beta1-pg19beta1-ts2\.27\.2-20260609|19beta1-pg19beta1-ts<timescaledb_version>-<yyyymmdd>)"
require(stable_immutable, "primary trixie immutable date tag is documented", "Document immutable trixie tags as {major}-pg{pg_version}-ts{timescaledb_version}-{yyyymmdd}.")
require(stable_bookworm, "secondary bookworm immutable tag uses -bookworm suffix", "Append -bookworm to secondary Debian immutable tags.")
...
require(pg19_immutable, "PostgreSQL 19beta1 uses experimental immutable tags", "Use immutable preview tags for PostgreSQL 19beta1 examples.")
require(r"imageName:\s*" + re.escape(repo) + r":(?:18|18-bookworm|18-pg18\.4-ts2\.27\.2-20260609|18-pg18\.4-ts2\.27\.2-20260609-bookworm|18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>|18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>-bookworm)(?:\b|>)", "CloudNativePG imageName examples use major-prefixed non-latest tags", "Use immutable or rolling major-prefixed imageName examples.")
...
reject_unless_negated(r"`latest`.{0,100}(?:points\s+to|targets|tracks|can\s+point\s+to|published\s+for|receives?|gets?|uses?|includes?|available\s+for).{0,100}(?:bookworm|18-bookworm|pg18\.4-ts2\.27\.2-20260609-bookworm|18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>-bookworm)", "latest is never documented for bookworm", "Use -bookworm tags without latest for secondary Debian images.")
...
reject(r"(?:bookworm|secondary)[^\n.]{0,120}18-pg<pg_version>-ts<timescaledb_version>-<yyyymmdd>(?!-bookworm)", "bookworm immutable examples include -bookworm suffix", "Append -bookworm to bookworm immutable tags.")
...
reject(r"18-pg<pg_version>-ts<timescaledb_version>(?!-<yyyymmdd>)", "immutable examples include UTC date suffix", "Use {major}-pg{pg_version}-ts{timescaledb_version}-{yyyymmdd}.")
...
if rg -n "\\b(17|18|19beta1)-pg(17|18|19beta1)(\\.[0-9A-Za-z._+-]+)?-ts[0-9][0-9A-Za-z._+-]*-[0-9]{8}(-bookworm)?\\b" \
  "${ROOT_DIR}/docs/image-tags.md" \
  "${ROOT_DIR}/docs/barman-plugin.md" \
  "${ROOT_DIR}/docs/catalog.md" \
  "${ROOT_DIR}/docs/user-guide/verifying-images.md" \
  "${ROOT_DIR}/cloudnative-pg-timescaledb/README.md"; then
  diag "rg concrete hand-written image examples" "public hand-written docs" "no concrete immutable production tags" "present" "Use tag-policy placeholders or generated release metadata references in hand-written docs."
  exit 1
fi
