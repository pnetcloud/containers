...
Use `make help`, `make update`, `make generate`, `make validate`, `make catalog`, and `make build PG=18 DEBIAN=trixie`.
...
| `cloudnative-pg-timescaledb/docker-bake.hcl` | `versions.yaml` | `make generate` | Commit validated output. | Do not hand-edit as final fix. |
| `cloudnative-pg-timescaledb/matrix.json` | `versions.yaml` | generator | Commit deterministic output. | Do not hand-edit as final fix. |
